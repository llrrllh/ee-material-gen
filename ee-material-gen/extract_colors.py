...
BROCHURE_DIR = Path("/Users/liuluheng/Library/CloudStorage/OneDrive-个人/Workfiles/00.招生相关/02.项目资料/课程画册")
OUTPUT_FILE = Path("/Users/liuluheng/.openclaw/workspace/ee-material-gen/courses_colors.json")
...
def extract_colors_from_pdf(pdf_path, max_pages=2, sample_step=4):
    ...
    # Try to identify dark background (look for near-black with slight tint)
    bg_dark = None
    try:
        ...
        for i in range(0, len(samples) - 2, 3):
            ...
            if 5 <= r <= 80 and 5 <= g <= 80 and 5 <= b <= 80:
                qr = (r // 8) * 8
                qg = (g // 8) * 8
                qb = (b // 8) * 8
                dark_colors.append((qr, qg, qb))
        doc2.close()
        if dark_colors:
